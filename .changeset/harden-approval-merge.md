...
"@docforge/actions": patch
---

Harden the approval gate: merge promotions through branch protection instead of `gh pr merge --admin`.

Adds `ForgeHost.merge(pr, opts)` — built on a pure `ghMergeArgs` helper that never emits `--admin` —
and switches the approval Action to a `GitHubHost` with auto-merge. The promotion commit is pushed
under a dedicated `FORGE_PROMOTE_TOKEN` so it re-triggers CI, then GitHub auto-merge completes the
