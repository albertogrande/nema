---
name: docs-author
description: Author a Nema documentation page end-to-end through the producer loop — draft with seeded provenance, self-check against the gates, and open a PR. Use when creating or substantially revising a docs page in a Nema repo.
---

# docs-author

Author a documentation page the Nema way: structured, provenance-tracked, human-gated.
...
3. **Draft** with the `draft_page` MCP tool (or `nema draft --path <p>`). Provide:
...
4. **Self-check**: run the `check` tool / `nema check`. Fix every diagnostic. Common ones:
...
5. **Propose**: `propose_changes` / `nema open-pr` with a clear title and summary. This opens a
   `nema/draft/*` PR with a `Nema-Provenance` commit trailer.
