---
description: Draft a new Forge docs page (status:draft + seeded provenance) and self-check it against the gates.
---

Draft a new documentation page through the Forge producer loop.
...
3. Use the `draft_page` MCP tool (or run `forge draft`) to write the page with `status: draft`
   and a complete `provenance` block (`authored_by: ai`, your `model`, the `sources`).
4. Run `forge check` (or the `check` tool) and fix every diagnostic.
